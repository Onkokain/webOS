export function fsDelete(filesystem, targetPath) {
  ...
  const pathsToDelete = Object.keys(updatedFilesystem).filter(
...
export function fsRename(filesystem, oldPath, newPath) {
  ...
  const pathsToRename = Object.keys(updatedFilesystem).filter(
...
export function fsCopy(filesystem, sourcePath, destinationPath) {
  ...
  const pathsToCopy = Object.keys(updatedFilesystem).filter(
...
export function fsNextName(
  ...
  while (true) {
    ...
    const numberedNameExists =
      filesystem[numberedPath] || filesystem[numberedPathWithSlash];
