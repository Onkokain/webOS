...
export function useWindowManager() {
  ...
  const openWindow = (kind, currentActiveId) => {
    ...
    setTree(prev => {
      ...
      if (SINGLE_WINDOW.includes(kind) && collectLeaves(prev, BOUNDS, null).some(l => l.kind === kind)) { idRef.current--; return prev; }
  ...
  const onTileHeaderMouseDown = (e, winId, tiledWindows) => {
    ...
    const onMove = (me) => {
      ...
      const over = tiledWindows.find(l => l.id !== winId && px >= l.bounds.x && px <= l.bounds.x + l.bounds.w && py >= l.bounds.y && py <= l.bounds.y + l.bounds.h);
    ...
    const onUp = (ue) => {
      ...
      setDragOverId(null); setDragPos(null); setDraggingId(null);
      ...
      const target = tiledWindows.find(l => l.id !== winId && px >= l.bounds.x && px <= l.bounds.x + l.bounds.w && py >= l.bounds.y && py <= l.bounds.y + l.bounds.h);
      if (target) { setTree(prev => swapIds(prev, winId, target.id)); setActiveId(winId); }
  ...
  return {
    tree, activeId, setActiveId, floating, registry,
    dragOverId, dragPos, draggingId, screenRef,
    openWindow, closeWindow, floatWindow, tileWindow,
    onTileHeaderMouseDown, onFloatHeaderMouseDown, onFloatResize,
    tiledWindows,
  };
}
