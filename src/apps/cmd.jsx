...
const run = (
  ...
  switch (baseCommand.toLowerCase()) {
    ...
    case "cd": {
      ...
      if (!target.startsWith(userHomeDirectory))
        return [`cd: permission denied: cannot go above /home/${user}`];
      if (target !== userHomeDirectory && !filesystem[target])
        return [`cd: ${commandArguments}: no such directory`];
    ...
    case "neofetch": {
      return [
        ...
        "Uptime: " +
          Math.floor(performance.now() / 3600000) +
          "h " +
          Math.floor((performance.now() % 3600000) / 60000) +
          "m " +
          Math.floor((performance.now() % 60000) / 1000) +
          "s",
        "User: " + user,
        "Hostname: " + HOST,
      ];
    ...
    case "ls": {
      const dir = commandArguments
        ? resolvePath(commandArguments)
        : currentWorkingDirectory;
      ...
      return entries.length
        ? entries.map(
            (k) =>
              k.slice(dir.length).replace(/\/$/, "") +
              (filesystem[k].type === "dir" ? "/" : ""),
          )
        : ["(empty)"];
    }
    ...
    case "mkdir": {
      ...
      if (filesystem[path])
        return [`mkdir: ${commandArguments}: already exists`];
    ...
    case "cat": {
      ...
      if (filesystem[path].type === "dir")
        return [`cat: ${commandArguments}: is a directory`];
      return (filesystem[path].text || "").split("\n");
    }

    case "rm": {
      if (!commandArguments) return ["usage: rm <path>"];
      const path = commandArguments.endsWith("/")
        ? resolvePath(commandArguments)
        : `${currentWorkingDirectory}${commandArguments}`;
      if (!filesystem[path])
        return [`rm: ${commandArguments}: no such file or directory`];
    ...
    case "fontsize": {
      if (!commandArguments)
        return ["usage: size <font size in px (default:14)>"];
      if (isNaN(commandArguments)) return ["Invalid size. Enter a number!"];
      if (commandArguments < 7 || commandArguments > 67)
        return ["Invalid size. Font Size can only be in the range [7,67]!"];
    ...
    case "browser": {
      ...
      const url = commandArguments.startsWith("http")
        ? commandArguments
        : "https://" + commandArguments;
    ...
    case "reset": {
      if (!commandArguments || commandArguments !== "confirm")
        return [
          'This will reset all user data. Type "reset confirm" to proceed.',
        ];
      return ["__RESET__"];
    }
...
export default function Cli({
  ...
  const submit = (e) => {
    ...
    if (out[0] === "__CLEAR__") {
      ...
      setHistory((h) => [
        ...h,
        { k: "dim", t: "Hackertype started!" },
        { k: "dim", t: "Press Esc to end." },
      ]);
      fetch("/webOS/hackertype.txt")
        .then((res) => res.text())
        .then((text) => {
          let i = 0;
          const handler = (e) => {
  ...
  const onKeyDown = (e) => {
    if (
      /*e.shiftKey && e.key === 'c' || will fix this logic soon */ e.key ===
      "Escape"
    ) {
      if (hackerActive) {
        clearInterval(hackerRef.current);
        setHackerActive(false);
        setHistory((h) => [...h, { k: "dim", t: "Hackertype ended!" }]);
      }
      return;
    }
    if (e.key === "ArrowUp") {
  ...
  return (
    <>
      ...
      >
        <div
          className="flex-1 min-h-0 overflow-y-auto px-3 pt-3 pb-1 font-mono hide-scroll"
          style={{ fontSize }}
        ...
        <form
          onSubmit={submit}
          className="shrink-0 row gap-2 px-3 py-2 border-t border-gray-800"
        >
          <span className="font-mono whitespace-nowrap " style={{ fontSize }}>
            <span>{user}</span>
            <span>@</span>
            <span>{HOST}</span>
            <span>:{shortCwd}$</span>
