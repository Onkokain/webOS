...
export default function Browser({ id, focused, onFocus, onClose, initialUrl,isDragging }) {
  ...
  useEffect(() => {
    localStorage.setItem('suprland-browser_history', JSON.stringify(history.slice(0,200)));
  }, [history]);
  ...
  return (
    <Window id={id} title="browser" focused={focused} onFocus={onFocus} onClose={onClose}>
      ...
      <form onSubmit={navigateActiveTab} className="flex-shrink-0 flex items-center gap-2 px-3 py-1.5 bg-[#111] border-b border-gray-800">
        ...
        <input
          ...
          onChange={event =>{
            if (!activeTab) return;
            updateTab(activeTab.id, tab=> ({...tab, input:event.target.value}))
          }}
