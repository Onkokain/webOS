...
export default function App() {
  ...
  const [settings, setSettings] = useState(() => {
    ...
    return saved
      ? JSON.parse(saved)
      : {
          wallpaper: `img:${BASE}wallpapers/anime.jpg`,
          hiddenApps: [],
          taskbarPos: "bottom",
          autoHide: false,
          textColor: "rgb(81, 230, 68)",
          fontSize: "16px",
  ...
  const openWindow = (window_type) => {
    ...
    setTree((prev) => {
      ...
      const windowAlreadyOpen =
        SINGLE_WINDOW.includes(window_type) &&
        existingLeaves.some((leaf) => leaf.kind === window_type);
  ...
  const closeWindow = (id) => {
    ...
    const isFloatingWindow = floating.some(
      (floatWindow) => floatWindow.id === id,
    );

    if (isFloatingWindow) {
      setFloating((prev) =>
        prev.filter((floatWindow) => floatWindow.id !== id),
      );
    ...
    setTree((prev) => {
      ...
      const nextActiveWindow =
        allLeaves[closingWindowIndex + 1] ?? allLeaves[closingWindowIndex - 1];
  ...
  const floatWindow = (id) => {
    ...
    setTree((prev) => {
      ...
      const floatingWindowIndex = remainingLeaves.findIndex(
        (leaf) => leaf.id === id,
      );
      const nextActiveWindow =
        remainingLeaves[floatingWindowIndex + 1] ??
        remainingLeaves[floatingWindowIndex - 1];
  ...
  const onTileHeaderMouseDown = (event, windowId) => {
    ...
    const handleMouseMove = (moveEvent) => {
      ...
      const cursorXPercent =
        ((moveEvent.clientX - screenRect.left) / screenRect.width) * 100;
      const cursorYPercent =
        ((moveEvent.clientY - screenRect.top) / screenRect.height) * 100;

      const allLeaves = collectLeaves(tree, BOUNDS, null);
      const hoveredWindow = allLeaves.find(
        (leaf) =>
          leaf.id !== windowId &&
    ...
    const handleMouseUp = (upEvent) => {
      ...
      const dropXPercent =
        ((upEvent.clientX - screenRect.left) / screenRect.width) * 100;
      const dropYPercent =
        ((upEvent.clientY - screenRect.top) / screenRect.height) * 100;

      const allLeaves = collectLeaves(tree, BOUNDS, null);
      const targetWindow = allLeaves.find(
        (leaf) =>
          leaf.id !== windowId &&
  ...
  const onTileHeaderTouchHold = (event, windowId) => {
    ...
    const handle_touch_over = (upEvent) => {
      ...
      if (screenRect) {
        const dropXPercent =
          ((touch.clientX - screenRect.left) / screenRect.width) * 100;
        const dropYPercent =
          ((touch.clientY - screenRect.top) / screenRect.height) * 100;
  ...
  const onFloatHeaderMouseDown = (event, windowId) => {
    ...
    const floatingWindow = floating.find(
      (floatWindow) => floatWindow.id === windowId,
    );
  ...
  const allKinds = [
    ...tiledWindows.map((w) => w.kind),
    ...floating.map((f) => f.kind),
  ];
  ...
  const winProps = (windowId, isFocused) => ({
    ...
    onSave: (fileName, fileData) =>
      saveFile(`/home/${user}/${fileName}`, fileData),
    fs,
    setFs,
    user,
    fmPath,
  ...
  const wallpaperClass =
    !isImgWallpaper && !isVidWallpaper
      ? settings.wallpaper.replace("color:", "")
      : "bg-black";

  const videoSrc = isVidWallpaper
    ? settings.wallpaper.replace("video:", "")
    : "";
  ...
  const startTouchPan = (e) => {
    if (!isPannableWallpaper || e.touches.length !== 1) {
      return;
    }
  ...
  const startpan = (e) => {
    ...
    if (!isPannableWallpaper || e.button !== 1 || hasWindows) {
      return;
    }
    e.preventDefault();
    setWppan(true);
  ...
  const taskbarHeight = settings.autoHide
    ? 0
    : settings.taskbarPos === "top" || settings.taskbarPos === "bottom"
      ? 85
      : 0;
  const taskbarWidth = settings.autoHide
    ? 0
  ...
  const contentStyle = {
    display: "none",
    paddingBottom:
      settings.taskbarPos === "bottom" && !settings.autoHide
        ? `${taskbarHeight}px`
        : "0",
    paddingTop:
      settings.taskbarPos === "top" && !settings.autoHide
        ? `${taskbarHeight}px`
        : "0",
    paddingLeft:
      settings.taskbarPos === "left" && !settings.autoHide
        ? `${taskbarWidth}px`
        : "0",
    paddingRight:
      settings.taskbarPos === "right" && !settings.autoHide
        ? `${taskbarWidth}px`
        : "0",
  };
            ...
            if (tiledWin) {
              ...
              return (
                ...
                >
                  <div
                    style={{
                      transform: isTarget ? "scale(0.8)" : "scale(1)",
                  ...
                  {isTarget && (
                    <div className="absolute inset-1 rounded-3xl border-2 border-cyan-400/60 bg-cyan-400/10 pointer-events-none z-50" />
                  )}
                </div>
              );
            }
            ...
            if (floatWin) {
              return (
                <div
                  key={id}
                  onMouseDown={(e) => {
                    if (e.button === 1 && tiledWin.kind !== "browser") {
                      e.preventDefault();
                      closeWindow(id);
                      return;
                    }
                ...
                >
                  <div className="w-full h-full relative">
                    ...
                    <div
                      className="absolute top-0 left-10 right-0 h-8 cursor-move z-10"
        ...
        {draggingId &&
          ...
          (() => {
            ...
            return (
              ...
              >
                <div
                  className="w-full h-full rounded-3xl border-2 border-cyan-400 bg-[#0a0a0f] flex items-center justify-center"
